...
public:
    MAVControl_Node() : Node("mav_control_node") {
        ...
        trajectory_setpoint_publisher_ = this->create_publisher<px4_msgs::msg::TrajectorySetpoint>(
            "/MAV2/fmu/in/trajectory_setpoint", qos_pub);
...
private:
    ...
    void cmd_cb(const std_msgs::msg::Float64MultiArray::SharedPtr msg) {
        ...
        RCLCPP_INFO_THROTTLE(this->get_logger(), *this->get_clock(), 1000,
                             "Velocity cmd: [%.2f, %.2f, %.2f] m/s",
                             velocity_cmd_[0], velocity_cmd_[1], velocity_cmd_[2]);
    }
    ...
    void velocity_cmd_calculate() {
        ...
        dbg.data = {
            static_cast<float>(velocity_cmd_[0]),
            static_cast<float>(velocity_cmd_[1]),
            static_cast<float>(velocity_cmd_[2])
        };
    }
